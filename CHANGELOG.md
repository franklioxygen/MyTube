...
- Reject a download yt-dlp left fragments out of. The audit above finds these after the fact; this stops new ones entering the library. The frame-count signal cannot be used at download time on its own, because it also fires on sources packaged with gaps, and rejecting on it would make those videos permanently impossible to download. yt-dlp, though, says when it gives up on a fragment - `Skipping fragment N ...`, once per fragment - and that is direct evidence the gap was introduced by this download rather than inherited from the source. The progress parser that every yt-dlp download path already runs its output through now counts those lines, and the completeness check rejects the file when any were reported, with the count in the failure reason. It applies to Bilibili, the general yt-dlp path and MissAV. A retry that went on to succeed is not counted, and a download whose output does not include the line - a user config running yt-dlp with `--quiet`, for instance - behaves exactly as before.

...
- Stop the MissAV discovery browser announcing a macOS User-Agent it cannot back up. A hardcoded `Macintosh; Intel Mac OS X 10_15_7 ... Chrome/136` string was forced onto Chromium, but Chromium does not rewrite everything to match: the `Sec-CH-UA-Platform` client hint, `navigator.platform` and the WebGL renderer all keep reporting the real platform, which in the Docker image is Linux. A macOS User-Agent arriving beside Linux client hints in the same request is a direct self-contradiction and one of the cheapest bot-detection signals there is, so the override made the browser easier to single out rather than harder. Chromium now sends its own string, which is consistent with everything else it reports.

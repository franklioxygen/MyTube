...
- Run the media integrity audit from Settings. `GET /api/media-integrity-audit` had no caller in the app, so finding a damaged video already in the library still meant calling the API by hand or playing the video until it broke. Settings → Library & Storage now has a Media Integrity Check section (admin only) that runs the audit on request and lists each finding with what was found, the recommended action and a link to the video. Where a re-download is recommended and the row has a source URL, Download Again queues the same forced download as the Manage page's re-download button; a row with no source URL says so instead of offering a button that cannot work, and, as on the Manage page, the button is not offered while that source is already downloading or queued, since a forced download always enqueues a new task and a second click would queue a duplicate writing the same replacement. Audit items now carry the row's `mediaType`, and the shared re-download hook sends `audioOnly` for an audio item: without it `POST /download` treats the request as video, so Download Again on a damaged audio item created a new video and left the audio item as it was. The same hook serves the Manage page's re-download button, which had the same gap and is fixed with it. Probing every file of a large library can take minutes, so the request waits as long as the `/api` proxy does (300 seconds), and a browser timeout or a proxy 504 is reported as a timeout rather than a failure: the server keeps scanning after the connection drops and caches each probed file for up to ten minutes, so running the check again soon after continues where it stopped. The result is held in the query cache so it survives switching settings tabs, and a failed scan is never retried automatically, since a retry would repeat minutes of work. The summary line is built in the browser from the audit's counts, because the response's `humanSummary` is English prose; it counts only the videos the audit actually opens. Each finding's detail text is still the server's English.

- Let the media integrity audit find content missing from the middle of a file, with `GET /api/media-integrity-audit?timeline=1`. yt-dlp's HLS and DASH downloaders skip a fragment that still fails after its retries and carry on, and the merged file keeps its timeline: the timestamps jump over the hole, so the duration is intact, the audio and video tracks agree, and neither existing check can see anything wrong. A scan of one production library found 16 of 351 MissAV downloads missing between 1.5 and 115 seconds this way, mostly exactly one 4-second segment, in files that had all passed the completeness check. Detection runs in two stages because the reliable signal is expensive. The frame count in each stream's header implies a duration, and when frames are missing the stream's duration exceeds it by about the missing span; reading that costs one ffprobe header call, and on a complete file the two agree to within milliseconds. Only a stream short by more than a second is then scanned packet by packet for a contiguous jump in its timestamps, and only a jump over a second is reported. The second stage is what keeps this from misfiring: some sources are published with a frame or two missing at every segment boundary, so their frame counts fall short by tens of seconds in total while no single step exceeds a few frames. That is how the source is packaged, a re-download reproduces it exactly, and it is not reported: the first stage flagged 25 files in that library and the scan cleared 6 of them. A contiguous jump is not always a failed download either - 2 of the 19 files reported had gaps that look like the source's own, including one that recurs every 16 minutes - and no threshold separates them, since real drops there ran as small as 1.5 seconds. So each finding says where the content is missing and for how long and recommends a re-download, and notes that a re-download showing the same gap means the source is missing it too. The scan reads the whole file, so it is opt-in rather than part of every audit, its results are cached by mtime and size for a day, and without it the audit's summary now says that mid-file gaps were not checked rather than implying a clean result.

### Fix

- Say so in the download history when yt-dlp left part of a video out. The audit above finds these after the fact, but a new one entered the library looking like any other success. yt-dlp says when it gives up on a fragment - `Skipping fragment N ...`, once per fragment - and that is direct evidence the gap came from this download rather than the source. The progress parser that every yt-dlp download path already runs its output through now counts those lines, and when any were reported the downloaded file is scanned to find the gaps. Its history row is then shown as incomplete, with a note, in the viewer's language, of how much is missing and where, and offers "Download Again", which replaces the file in place. Plain retry is not offered, because resubmitting the URL is skipped once the video exists. The video is kept rather than rejected. It plays, jumping over the gap, and in the library that prompted this about one download in twenty lost a fragment, so rejecting would mean re-fetching a whole multi-gigabyte file for a few seconds - and, for a fragment that is gone from the CDN for good, would make the video impossible to save at all. Underneath, the row is still a success, because subscription retention, filename-template renames and deletion tombstones all key on that status and the video is in the library. The note rides in the row's `error` field as JSON rather than a sentence - the fragment count and each gap's stream, position and length - so the client can word it in every supported language, following the pattern of the Bilibili retry metadata. The history view and its filter treat a success carrying one as incomplete. The downloader records the note against the video it saved, and whichever caller writes the history row - the download manager, a subscription or a continuous download - picks it up there. Every save sets or clears it, so a note left by an earlier attempt cannot attach to a later clean download. It applies to Bilibili, the general yt-dlp path and MissAV. A retry that went on to succeed is not counted, and a download whose output does not include the line - a user config running yt-dlp with `--quiet`, for instance - behaves exactly as before.

- Back off between fragment retries on MissAV. yt-dlp retries a failed fragment 10 times back to back, with no pause, before leaving it out, and a CDN hiccup of a few seconds is enough to exhaust that. MissAV downloads now retry a fragment 20 times, waiting 1 second and doubling to a 20-second cap, about five and a half minutes before giving up on it. That rides out a transient failure, which is how most of those one-in-twenty gaps came about, and still gives up on a fragment that is gone. A user's own `--fragment-retries` or `--retry-sleep` takes precedence. MissAV is the only path changed: it builds its yt-dlp flags explicitly, and it is where the gaps were measured.
...
- Surface download artifacts abandoned in the library. `POST /api/cleanup-temp-files` removes `.part`/`.ytdl` files and temp directories carrying the ownership marker, but two kinds escape it entirely: temp directories created before that marker existed, which are unrecognisable to the ownership check and can never be cleaned however often the endpoint runs, and the `*.fNNN.*` / `*.temp.*` split-stream files the non-Bilibili downloaders write straight into the library and leave behind when a merge fails. Nothing surfaced either, so the floor only ever rose - one production library held 565 MB across ten of them, including a 136 MB directory that had to be deleted by hand. A sweep now identifies these artifacts along with abandoned marked temp directories and reports what it finds shortly after boot. It removes nothing: these artifacts run to hundreds of megabytes and clearing them unprompted is not a restart's decision, and the cleanup dialog currently discloses only `.part` and `.ytdl` removal. Every candidate must clear three independent conditions - a known artifact name shape, older than seven days, and referenced by no library row - so one check misfiring cannot select a file still in use, and a user folder named `temp_holidays` does not match the directory pattern.
...
- Take a source duration for MissAV from its HLS playlist. It was the one download path passing no source duration, so only the audio/video track comparison applied to it and a download shortened equally across both tracks went unnoticed - which, since yt-dlp defaults to `--skip-unavailable-fragments` for native HLS, is the failure mode most likely to occur there. It reads only the playlist bodies the Puppeteer step already received, and issues no request of its own: the m3u8 host sits behind Cloudflare bot management that fingerprints the TLS handshake - the same reason the download itself needs `--impersonate` - so a plain Node request for the same URL is answered with 403, which made a direct fetch unreachable on precisely the host it mattered for. Reading only what the browser already has needs no origin policy, no redirect handling and no timeout, because a hostile playlist has nothing to point it at. The playlist is a better source than the page: it describes the exact stream being fetched, so it cannot disagree with the rendition yt-dlp downloads, whereas a page-scraped number describes the title. A master playlist is corroborated rather than guessed at: yt-dlp, not this code, selects the rendition that gets downloaded, so up to two are read and compared. Two that disagree by more than the completeness check itself tolerates make the master unusable, and otherwise the shorter of the two is taken, since understating the source is the direction that cannot produce a false rejection. When only one rendition can be read - the usual case on a CDN that answers anything the browser did not fetch with a 403 - it is used as it stands, because reporting no duration at all would disable the check on exactly those hosts, and the tolerance of max(10s, 5%) is far wider than renditions of one VOD realistically differ by. A rendition the browser did not fetch is simply skipped, at no cost, since this is a lookup rather than a request. A master offering separate audio or subtitle renditions is refused outright, because each variant then covers only part of the muxed result. `EXT-X-ENDLIST` is required because a live or growing playlist cannot establish the final duration. Malformed playlists and failed requests also yield no duration, preserving the track comparison as the fallback. The playlist request honors the download's proxy setting, including an explicit direct connection.

- Reject a MissAV run that exited cleanly but produced no file. The completeness check is deliberately fail-open, reporting a file it cannot read as unknown rather than broken so a host without ffprobe keeps working, which means it cannot answer "nothing was produced at all". Bilibili and the yt-dlp path gate on that separately; MissAV did not. Its only existence test was for file size, and when that failed it left the size undefined and saved the row anyway, so a run that wrote nothing produced a library row pointing at a file that never existed. Pre-existing, and not something the completeness check covered, though its presence could be mistaken for covering it.

- Reject a download whose media streams were cut short instead of storing half a video. A transfer that dies partway still leaves yt-dlp with two files to merge, so the merge succeeds, the container is valid, ffprobe reads it and MyTube saved it as a normal library item - one that plays cleanly until it reaches the truncation point and then dies there, which Chromium reports as `PIPELINE_ERROR_DECODE` on the first garbage packet. Nothing downstream could catch it, because the duration written to the database is measured from that same truncated file: an 1111-second Bilibili video whose video track stopped at 685s and whose audio track stopped at 400s was recorded, correctly as far as the file was concerned, as 685 seconds long. yt-dlp's own metadata knew the real duration all along and was discarded. Every downloaded file is now probed before it is published and rejected on either of two signals: it is materially shorter than the duration the source reported, or its audio and video tracks disagree about how long it is. The second matters on its own - a merge of one complete and one truncated stream keeps the container duration honest, which is how a 44-minute video carrying six minutes of audio passed as intact. Both tolerate a 5% drift with a 10-second floor, so ordinary muxing slack, a rounded source duration and short clips are unaffected, and both skip themselves when the input is unknown: a source that reports no duration (a live stream), a container that stores no per-stream durations (Matroska and WebM routinely do not), an audio-only download with no video track to compare against, a config that removes content deliberately (`--download-sections`, SponsorBlock removal, chapter removal), and a host where ffprobe is missing or fails all leave downloads behaving exactly as before. The check may only reject a file it has positively measured as truncated. A rejected download is discarded before it reaches the library - and, on a re-download, before it can replace the file it was meant to improve - and recorded as a failed download with the measured reason, so it can be retried rather than sitting in the library looking fine. Rejection cleanup targets the paths this attempt actually allocated, never a filename recomputed from title and author: under a collision suffix or a filename template that reconstruction can name another library item's file. Each download branch stops treating its output as disposable the moment its database row starts referencing it, so a later organization, export or identity failure - or a stale cancel callback - can no longer delete published media and leave a row pointing at nothing. Every download path gained it — Bilibili, the yt-dlp path that serves YouTube and the rest, and MissAV — since all three had the same gap. MissAV scrapes no duration from its source, so only the track comparison applies there; it is passed an explicit null rather than a guessed value, and that is the check which catches a one-sided truncation anyway. The Bilibili log filter also stops swallowing ffmpeg's "Invalid data found when processing input", which is precisely the line that reports a stream ending mid-sample: it was listed as an expected warning to skip while a comment sixteen lines below it recorded that it is a real error, so the one message that would have explained this never reached the log.

### Refactor

- Delete the unreachable `SearchResults` page. It was the original `/search` view, rendered by `AnimatedRoutes`; when `SearchPage` took over that route the `AnimatedRoutes` wrapper itself fell out of use, and the commit that later deleted the wrapper removed the last `<SearchResults />` reference along with it, leaving the page and a 417-line test behind with nothing rendering either. A test was in fact added to it afterwards, which is the clearest sign the file read as live code. Nothing is lost by removing it, because `SearchPage` had already superseded it on every axis: it reads its query from `?q=` rather than from context search state, sorts the local results through `SortControl`, and runs its strings through `t()` where `SearchResults` still carried hardcoded English. What makes it worth deleting rather than leaving in place is that a second copy of the result grid does not stay harmless - the Bilibili search work refactored `SearchPage`'s online sections into `ExternalSearchSection` and added a source beside YouTube, and none of that reached this file, so the dead copy kept drifting further from the live page with every change to search while still reading like an alternative implementation of it. The behavior unique to it - resetting the search on an empty term, clearing search mode, and the full-page "Searching for..." splash - belonged to the context-driven search-mode scheme that `/search` no longer uses.

## v1.11.9 (2026-09-13)

### Fix

- Retry a subscription video that the initial backfill failed to download. A playlist subscription seeds its cursor to the collection head at creation time, before the task that downloads the history has run, so a scheduled check does not fetch the same videos in parallel. If that task then failed on the head - one transient yt-dlp timeout is enough - the cursor was already past a video nothing had downloaded, every later check saw `head == lastVideoLink` and concluded there was nothing new, and the gap was permanent: the collection stayed one episode short, with no error left anywhere to explain it and no amount of waiting to fix it. A failed video on a subscription-linked task now clears that subscription's cursor, but only while the cursor still points at that video, so a check that has meanwhile moved on to a genuinely newer upload is not rewound underneath itself. Nothing else about failure handling changes: the scheduled check already retried an ordinary download failure by leaving the cursor where it was, and this extends the same treatment to the backfill. A check that finds its head already in the library now links it into the subscription's collection - at the front when the backfill ran newest-first, so a recovered head does not land behind every older video - and advances the cursor instead of downloading a second copy - and carries on to the independent Shorts check rather than returning, since advancing the cursor also refreshes the check time and returning would have hidden a new Short for a full interval. The lookup accepts a Bilibili part-one URL under either of its forms, since an all-parts download stores it as `?p=1` while the probe returns the bare URL, and is scoped to the media type the subscription's effective configuration would save under, the way the backfill's own duplicate check already is - which is what makes the rewind safe, since the retried video is fetched when it is missing and quietly settled when some other path has since supplied it.

- Send the stored cookies on every Bilibili API request, not only the ones the subscription probe makes. v1.11.8 attached them where a stalled subscription had made the need visible, but the same hand-rolled `Referer` + truncated user agent remained at seven other call sites, and api.bilibili.com answers 412 to a cookieless `x/web-interface/view`. Two of those were failing on every Bilibili download in the field: the author avatar lookup, so the avatar silently never resolved, and the part-title lookup, so a multi-part video fell back to yt-dlp's whole-video title instead of the part name. The rest - the space-listing fallback, the WBI key fetch, the continuous-download space enumeration, and the channel-URL backfill - share the defect and are fixed with it rather than left to surface one report at a time. The subtitle reader had a second cookie implementation of its own that sent every cookie in the file to Bilibili, YouTube's included, and dropped any exported with the `#HttpOnly_` prefix - `SESSDATA` among them, which is the one it warns about needing. It now uses the shared builder, called separately for the view and player endpoints, since the builder scopes cookies to the exact URL and one header set shared between the two would send view-scoped cookies to the player path and drop the ones scoped to it - and the player response is what carries the subtitle URLs. That reader is deleted. The builder moved to its own module so a request only needs the cookie jar, not the settings and database its previous home pulled in.

- Stop the settings page mixing input and text widths. Each section had picked its own numbers, so controls landed on 240, 300, 400 and 420 across the page - two components had even grown private `SELECT_MAX_WIDTH` constants, both set to 400, neither aware of the other - while descriptions were either uncapped, running the full 1152px container beside the 400px input they described, or capped at 520, 640, 760, 920 or 960 depending on who wrote them. A new `settingsLayout.ts` holds the two widths the page actually needs: `SETTINGS_CONTROL_MAX_WIDTH` for inputs and selects, and `SETTINGS_SECTION_MAX_WIDTH` for the readable column, which `SettingsPage` applies once around the tab content and each mobile section so no section restates it and a new one needs no width of its own. Both numbers were already in use in the codebase; neither is new. The stragglers this exposed are aligned with it: the auto-retry pair spanned wider than the single select beneath it, the concurrency slider began 16px inside the column edge because its wrapper's padding ate into the width rather than adding to it, the Telegram, Twitch, cloud-drive and live-translation credential fields had no cap at all, and the three author-organization radio options each wrapped at their own natural width so their rows ended on different edges. The yt-dlp configuration textarea keeps the full column deliberately - it is a code editor. Measured afterwards on every tab, controls share one left and right edge and prose shares one measure, and nothing overflows at 375px.

### Refactor

- Regroup the settings page by what each setting actually does. The six tabs had drifted badly out of balance - Download & Storage and Security & Access carried roughly 2500 lines of components each against Content Management's 500 - and, more importantly, out of theme. File naming and layout lived in two places: the filename template and media-server export under Download & Storage, while the author folder mode, subtitle and thumbnail placement and the legacy rename that consume them sat under Data Management. Third-party credentials were scattered over five tabs with no integrations home. Names had stopped matching contents: Data Management was a single `DatabaseSettings` component of which three of six sections were file layout rather than database, Security & Access held the RSS feed output, and Basic Settings held an AI feature with its own API key - which is also why it had grown so heavy, since `interfaceDisplay` and `videoPlayback` had once been top-level tabs and were folded into it, their category keys still sitting in the locale files. The tabs are now Interface & Display, Video Playback, Downloads & Files, Library & Storage, Security & Access, and Integrations & Advanced, rendered from one array so the desktop tab strip and the mobile accordion cannot drift apart. Statistics moves beside the other privacy controls, since it toggles visitor tracking, raw search capture and retention. The file-layout controls split out of `DatabaseSettings` into a new `FileOrganizationSettings`, placed beside the filename template that produces the names they rearrange. Deep links are fixed with it: `?tab=` carried the tab's raw array position, so every past regrouping silently repointed existing links, and both in-app links were already wrong - the statistics link pointed at `?tab=7`, an index that no longer existed and fell back to the first tab so its anchor never resolved, and download history's "Change Settings" pointed at Data Management for a setting that lives in Download. The page's own test had hidden the second by stubbing that anchor inside the `DatabaseSettings` mock, so it manufactured the element it asserted on and passed while the real link was broken. `?tab=` now carries a stable key; old numeric links still resolve.

- Remove the duplicate legacy filename rename. "Format Legacy Filenames" and "Rename existing files to match the current naming pattern" did the same job, which regrouping the tabs made plain by putting them side by side. They were not merely similar: with the naming mode set to legacy the newer action takes a branch that calls the very same `formatVideoFilename()` the older one used, and is documented as producing byte-identical output. The newer action is a strict superset - it also handles template mode, which the old one ignored entirely, runs as a cancellable background job with progress instead of a synchronous request that blocks until every file is renamed, refuses to start while downloads are active or queued rather than renaming files out from under them, and moves thumbnails and subtitles per the current settings. The removed action also carried a defect the newer one already guards against: the renderer strips a previously-applied suffix before reformatting, because without that a second run concatenates - `X-Yajun-2026` cleans to `XYajun2026` and reformats to `XYajun2026-Yajun-2026.mp4` - and the old path passed the stored title straight in with no such guard, so running it twice corrupted filenames. Gone with it are the button and its confirmation modal, the mutation behind it, the `POST /settings/format-filenames` route and controller, `storageService/legacyFilenames.ts` whose only export it was, seven now-unused keys from all ten locales, and the endpoint from both API documents. No capability is lost: set the naming mode to legacy and use the template rename, which is never disabled in that mode.

### Chore

- Regenerate `KEY_LIST.md` and stop it drifting. The file calls itself derived from `en.ts`, but nothing derived it, and it had drifted three separate ways at once: it stated 1091 keys, its tables listed 1225, its own per-section counts summed to 1034, and the real number was 1371, with seven whole sections added to `en.ts` over time and never added here. A hand-maintained derived artifact that nothing checks is worse than none, because it is trusted - so rather than correcting the stated total and leaving 146 keys unlisted, `keyListDocument.ts` now builds the document from `Object.keys(en)` for the authoritative order plus the comment headings in the `en.ts` source for grouping, taking keys from the module rather than the text so nested object keys and lookalike lines inside template strings cannot inflate the list. A case in the existing `localeParity.test.ts` asserts the committed file equals that output and rewrites it instead when `UPDATE_KEY_LIST` is set, reusing the suite that already guards locale parity rather than adding a script, dependency or npm entry.
...
- Stop reporting a Cloudflare block on pages Cloudflare never blocked. The challenge check matched a bare `challenge-platform`, but Cloudflare injects `/cdn-cgi/challenge-platform/scripts/jsd/main.js` into every page it fronts, challenge or not, so any ordinary MissAV page matched. That check runs only after no m3u8 was captured, which turned every "the player never started" failure into "Cloudflare is blocking you" and sent diagnosis in the wrong direction - reported from the field, where a page that loaded fine and yielded its title still failed with the Cloudflare message. The pattern now matches only markers belonging to the interstitial itself: the challenge script's config object, the orchestration endpoint as opposed to the bot-management beacon, the interstitial's own markup and copy, and a Turnstile widget. `security verification` is tightened to the interstitial's full `Performing security verification`, since the short form also matched ordinary page text. The error raised when no stream is found no longer names a cause at all, in either direction: it cannot know Cloudflare was involved, and it cannot know the page was served normally either, since an origin error page or a WAF denial carrying none of these markers reaches the same branch. `navigateMissAvPage` now returns the main response's HTTP status, so the message reports that one observed fact and leaves the rest to the HTML it was already dumping. The status is withheld when it no longer describes the page in hand: a Cloudflare interstitial answers 403 and then navigates again on its own once it clears, so quoting the captured status there would blame a block that had already been got past. That case reports the cleared challenge instead, which is the more useful fact anyway.

## v1.11.5 (2026-09-04)

### Added

- A "Bypass proxy for hosts" setting under yt-dlp Configuration, for deployments that reach the internet through an outbound proxy. MyTube spawns yt-dlp with the backend container's own environment, so an `HTTP_PROXY` set there applies to every request yt-dlp makes with nothing in the UI to say so; the listed hosts are folded into the child's `NO_PROXY` instead, which is the only lever that discriminates per request host. Entries are normalized the way both of yt-dlp's HTTP backends actually match them - by domain suffix, so `surrit.com` already covers `cdn.surrit.com` - and a leading `*.` is stripped rather than left to silently never match, since neither backend treats it as a wildcard. The lowercase spelling is treated as authoritative and the result written to both, because CPython's `getproxies_environment()` reads the environment twice and its second pass matches lowercase names only: a non-empty `no_proxy` overrides `NO_PROXY`, and an empty one removes the bypass list outright - which is how a deployment clears a `NO_PROXY` inherited from a base image, and merging the union of the two would have resurrected exactly those entries. The setting carries the same "container" trust gate as the raw yt-dlp configuration it sits beside, since its value decides which traffic leaves the container unproxied, and it has no effect when `--proxy` is set in that configuration - yt-dlp then ignores `NO_PROXY` entirely. It governs yt-dlp's own egress: MyTube's side requests go out through axios, which reads `NO_PROXY` from the backend process rather than from the snapshot handed to the child, so a host that must be off the proxy for everything belongs in the container's own `NO_PROXY`, which every stack honours. `documents/{en,zh}/docker-guide.md` gain a section on outbound proxies covering all of this, including the `NO_PROXY` entries that look right and do nothing.

### Fix

- Stop a failed MissAV metadata lookup leaking a Chromium process, and stop it reporting a Cloudflare challenge as a successful lookup. `getVideoInfo` launched the browser and closed it on the happy path only, so anything thrown out of navigation - which is exactly what a Cloudflare challenge does - orphaned one Chromium per attempt, and a run of failures is precisely when they pile up; the close now happens in a `finally`, as the download path already did. The challenge itself was also invisible on that path: a challenge page parses fine, it simply carries no `og:` tags, so it was indistinguishable from a video page with no metadata and the placeholder title was returned as though the lookup had worked. The page HTML is already in hand there, so it is now checked against the same challenge markers the download path uses - which, unlike the exact-title probe during navigation, catches a challenge whatever it is titled. The lookup still returns placeholder metadata rather than throwing, since callers depend on that, but the reason is now in the log.
- Stop the MissAV discovery browser announcing a macOS User-Agent it cannot back up, without announcing a headless one instead. A hardcoded `Macintosh; Intel Mac OS X 10_15_7 ... Chrome/136` string was forced onto Chromium at launch, but Chromium does not rewrite everything to match: the `Sec-CH-UA-Platform` client hint, `navigator.platform` and the WebGL renderer all keep reporting the real platform, which in the Docker image is Linux, so the override put a self-contradiction in every request. Simply dropping it is not the answer either, because headless Chrome's own string says `HeadlessChrome/<version>` - a louder automation signal than the mismatch was, and the default in a container. The User-Agent is now derived from the running browser with only that marker rewritten, so the platform and version stay honest, agree with the client hints sent beside them, and nothing announces the headless session.
- Stop the MissAV Cloudflare error recommending something that cannot work. It told the operator to retry with `PUPPETEER_HEADLESS=false`, which needs a display the container does not have. It now says what is actually happening - the egress IP is being challenged, not this particular video - and points at the two things that help: Auto Retry, since a later attempt often lands on a different exit and succeeds, or a cleaner egress.
- Stop `MYTUBE_DATA_DIR` relocating the backend's data directory. v1.11.4 taught the backend to read that variable as the directory holding `mytube.db`, but the name already belongs to the *host* side of the `<host>:/app/data` bind mount in `stacks/docker-compose.single-container.yml` and the Docker guide, and `entrypoint.sh` has read it as a container path since v1.9. Where the variable reaches the container's environment - an `env_file`, or a NAS or Portainer stack that passes its variables through - the backend resolves a host path such as `../data` or `/volume1/docker/mytube/data` inside the container, finds no database there, and opens a brand new one; nothing fails, because the entrypoint has already created the directory. A fresh database carries default settings, `loginEnabled` defaults to false, and `isLoginRequired()` then reports that no login is needed - the single condition every route guard checks first - so an upgrade could bring a password-protected instance back up open to anyone, with an empty library. The backend override is now `MYTUBE_BACKEND_DATA_DIR` and `MYTUBE_DATA_DIR` is ignored again, so the data directory resolves to `<backend cwd>/data` exactly as it did in v1.11.3. `entrypoint.sh` reads the same variable, so the directory it prepares and chowns is the one the backend opens; it no longer reads `MYTUBE_UPLOADS_DIR` either, which is the host side of the uploads mount in exactly the same way and, when it leaks in, aims the startup permission pass at a directory nothing serves from while leaving the real `/app/uploads` alone. Because the rename is itself a breaking change for anyone who had already set `MYTUBE_DATA_DIR` as a container path - and its failure mode would be the same silent one - the migration runner now refuses to start when the data directory it resolved holds no database while a populated `mytube.db` sits at a directory the deployment plausibly meant instead, naming that file and the variable to fix. Silence is the whole danger here: an empty database migrates cleanly and the server reports a healthy start, so nothing but login protection quietly being off says anything is wrong. A first install, where no other database exists, is unaffected.

- Stop MissAV downloads crawling behind an outbound HTTP proxy (issue #446). MissAV streams are Cloudflare-protected, so they are fetched with yt-dlp's browser impersonation, which forces yt-dlp's native HLS downloader; that downloader honours `--concurrent-fragments`, which defaults to 1, so a stream's several hundred fragments were fetched strictly one after another and the per-request round trip - not bandwidth - decided throughput. A proxy adds a round trip to every one of them, which is why the same download that ran at full speed direct fell to a fluctuating crawl through mihomo. The MissAV flag set is assembled from the network-config allow-list, which carries no `-N`, so a user's own `--concurrent-fragments` never reached this path either and could not be used as a workaround; it is honoured now, and four fragments are fetched in parallel by default. A non-numeric value is ignored rather than forwarded, since yt-dlp rejects the whole invocation on one and that would break downloads that work today.
- Make "Proxy only apply to Youtube" apply to proxies it does not own. The setting deleted the `--proxy` flag for non-YouTube URLs, which does nothing about `HTTP_PROXY`/`HTTPS_PROXY` in the environment: yt-dlp reads those itself, so a deployment that sets them went on proxying every non-YouTube download while the UI said otherwise. Non-YouTube URLs now get yt-dlp's explicit direct-connection value (an empty `--proxy`), which overrides the environment, and the rule runs even when no yt-dlp configuration text is set at all - that is precisely the deployment where the proxy comes from the environment rather than from the config. The empty value is meaningful rather than absent, so the two places that split network options out of the user config forward it instead of dropping it as falsy. It also has to reach axios, which reads `HTTP_PROXY` from the environment on its own: `getAxiosProxyConfig` now maps it to `{ proxy: false }`, and every side request that accompanies a download - Bilibili API metadata, subtitles and avatars, the author-channel lookup, the short-URL redirect chain, the XiaoHongShu profile scrape, and thumbnail fetches on the MissAV and generic yt-dlp paths - passes it through rather than treating it as absent. The last two of those reached the network with no proxy configuration at all, so they now honour a configured proxy as well, and skip rather than send the request unproxied when that proxy turns out to be unusable, so a request can no longer stay on the proxy the download it belongs to has just left. A proxy that is genuinely unset still means "nothing configured", not "go direct". The MissAV page load that discovers the m3u8 URL gets the same treatment: Chromium reads `http_proxy` from the environment too, so it is launched with `--no-proxy-server` when the direct value is set - otherwise a proxy that is slow or cannot reach MissAV failed the download at the browser step, before the direct yt-dlp invocation was ever reached. **Behaviour change:** if you enabled this toggle but relied on a container-level proxy to reach non-YouTube sites, clear it.

## v1.11.4 (2026-09-03)

### Added

- Gesture Login: an admin-only 3x3 pattern that unlocks the login page without typing a password. The pattern is canonicalized by a module that exists in identical form on both sides — the frontend keeps a stroke canonical as it is drawn, the backend re-canonicalizes every request and is authoritative — with Android's midpoint rule derived from the grid geometry rather than a hand-written table. The credential is a scrypt hash in a singleton table, peppered from `GESTURE_LOGIN_PEPPER` or, when that is unset, from a file created beside the database with exclusive-create semantics and mode 0600, so two concurrent first-time enrolments cannot each mint one; neither `JWT_SECRET` nor `CSRF_SECRET` is reused, since both fall back to a per-process value that would invalidate an enrolled gesture on every restart, and a truncated pepper file is reported unavailable rather than regenerated over a working credential. The public status endpoint resolves the pepper read-only, so polling it can never mint a secret. Password login stays available as the only way out of a gesture lock: it cannot be disabled while a gesture exists, the check fails closed when the database cannot be read, and it is re-read after hashing because scrypt yields to the event loop. Enrolment is a compare-and-swap rather than an upsert, so a concurrent change — including a lock taken while the new gesture was being hashed — is reported instead of being clobbered. Failures are shared across devices: one or two expire 12 hours after the latest, three lock gesture access until an admin signs in with their password.
- Add a Clean Up button to the Collections tab of Manage Content, for deleting collections that hold no videos. A library gathers empty collections over time - a scan that grouped a folder later emptied by hand, a playlist whose videos were deleted - and clearing them one row at a time was the only way to be rid of them. The button says how many are going and waits for confirmation before anything is deleted, and is disabled while there is nothing to clean. Each collection goes through the same path as a single delete, so a leftover collection folder on disk is tidied up the same way. New endpoint `DELETE /api/collections/empty`, documented in `documents/{en,zh}/api-endpoints.md`: it decides emptiness itself rather than taking a list from the caller, so a collection filled by a concurrent request is judged on what the database holds, and it is registered ahead of `DELETE /api/collections/:id` so `empty` is never read as a collection id. Write access is gated exactly as the existing collection delete is, and a failed read of the collection list is raised rather than answered with an empty one, so a cleanup that could not run is never reported as one that found nothing to do. Deleting a video already cascades its collection membership away in the database, but the cached collection list was not refreshed with it, so a collection emptied that way went on listing an id that was gone and did not read as empty here; it is now invalidated alongside the video, single and bulk deletes alike. Deleting rows can leave the table on a page past the end of the list - an empty table with the pager gone - so the tab now pulls the page back into range, which covers deleting a single collection from the last page as well. The tab's video-count column now shows the number alone instead of appending an untranslated "videos".

### Fix
- Stop a caller-supplied URL being read as yt-dlp's own command-line options. Every yt-dlp invocation appended the URL as the last element of the argument list with no `--` separator before it, so a value beginning with `-` was consumed by yt-dlp's option parser as a flag rather than as a download target - the child process runs with `shell: false`, so this was never shell injection; the parsing that mattered is yt-dlp's own. A single injected token replaces the URL entirely, so nothing downloads and `--exec` on its own never fires, but a yt-dlp configuration file supplies the positional URL as well as options, so a `--config-location` pointing at a file an authenticated user had already placed on disk would have restored a real download target alongside it. The end-of-options marker is now emitted immediately before the input at all five places the argument list is built - the four in `utils/ytdlp/execute.ts` and the MissAV m3u8 download - which are the only callers that hand a caller-supplied value to the binary, so nothing reaches the parser as an option again whatever validated it upstream. URL validation - an http/https scheme and the private-host blocklist - also runs now on the two endpoints that were still forwarding a raw value, `POST /downloads/channel-playlists` and `GET /check-playlist`; `POST /download` already validated. Reported by Abdurazzoqov Javohir (GHSA-hq26-56gg-qx49).
- Let the left and right arrow keys page through a collection the way they already do on Home. Home's pagination hook owned that keyboard handling, and a collection keeps its page in local state rather than in the URL, so it never got any: the only way through a long collection was the pager at the bottom. The handling is lifted into a shared hook both now use - same guards, so a keypress while typing in a field, or on a collection short enough to fit one page, still does nothing. An open dialog or menu keeps the arrow keys as well, since paging the grid behind one means closing it reveals a page nobody asked for.

- Stop mount-path validation rejecting names that merely contain `..`. The guard was a `path.includes("..")`, so any legitimate file or folder with two consecutive dots read as traversal: scanning a mount directory that held one failed the whole request with a 500, and files like `Cat's in the Bag... .mkv` were skipped by the scan and refused at playback. Real traversal is a path *component* equal to `..`, which the segment-based check the codebase already had covers, and `normalizeSafeAbsolutePath` runs right after each of these guards anyway; the five mount call sites now use a segment check.
- Stop the mount scan importing what a media server keeps beside the film. QNAP writes generated preview clips into `.@__thumb` with the same extensions as the originals — on one 2764-video library 618 of the imported records, 22%, were preview clips, 154 of them carrying a bogus TMDB match — so dot-entries and the conventional NAS system folders (`@Recycle`, `@eaDir`, `#recycle`) are skipped. Trailers, samples and featurettes are filtered out of the scan's file list rather than merely discounted later, which also keeps them out of the on-disk set, so extras a previous scan imported are cleaned up by the next one. The filename markers are deliberately narrow — the media-server convention's hyphen (`Film-trailer.mkv`), the whole-name form, and `.sample.` — because accepting any separator made an ordinary final title word mark the film itself as bonus material (`The.Interview.mkv`), which dropped it from the scan and deleted the record it already had. Mount records the scan does not find on disk are now purged even when their directory has been removed from the setting, where before they were orphaned in the database forever; only the row goes, since `deleteVideo()` treats `mount:` paths as unmanaged.
- Stop a media library turning into hundreds of one-video collections. Every film has its own folder, so a 183-film library produced 183 collections and buried the ones that meant something. Videos per folder are counted before processing, so the decision no longer depends on scan order, extras are discounted from that count (a film with three featurettes is not a set), and a lone film now gets no collection at all rather than being joined to an existing one of the same name — 161 such collections were being quietly re-entered by later scans. Genuine sets still group. A collection created for a season folder is titled after the work TMDB recognized instead of the raw release name, keeping the folder as `name` so lookups still match, and only when that folder is the collection folder, since with a library root as the scan root the first segment is the library name. Episodes keep their designator (`医院五日 - S01E01`) instead of eight files sharing one show title, read off the filename directly so plain `S01E01.mkv` names work as well as `Season 1 Episode 2` ones. Membership is also reconciled after the pass for files the scan skipped as unchanged and for files it refreshed, either of which previously left a folder's first arrival out of the collection its second created.
- Stop a mount scan stalling the server. Linking each scanned video to its collection went through the legacy relocation path, which reloads every collection joined against every membership row, once per video, synchronously — partway through a 2764-video library that is millions of hydrated rows per file, and the whole server stops answering for the length of the scan. Mount media belongs to the media server and must never be relocated, so mount scans skip that branch entirely; local scans are unchanged.
- Recognize far more films when scraping a media-server library. The filename parser left release tags in the title, so the query was never usable (`All Quiet on the Western Front NF 1600MB DD5`): a codec glued to its channel count has no word boundary to match on, and nothing removed streaming-service tags, bit depth, frame rate, channel counts, file sizes, hyphenated `Blu-ray`, AVC, REMUX, EXTENDED, Criterion Collection, MiniSD, audio-track counts or the language and hardcoded-subtitle markers beside them. Those are stripped before any candidate is built, and because trailing junk is rarely one word the shortened candidate is offered repeatedly, up to three words. The matcher took the first result whose title merely contained the query, which matched making-of features that share a film's year; it now scores candidates, exact beating contained. `language` only changes the strings TMDB returns, never which results come back, so when the localized pass finds no confident match the same search is fetched in English and matched against those titles too, keeping the localized result for display — `Anatomy of a Fall` comes back as 坠落的审判 with a French original title and matched neither before. Where the filename still misses, mount scans retry with the folder that names the work, walking past season and extras folders and stopping at the scan root, since on a media-server library the identity lives on the folder. Cleaner queries reach further, which surfaced a new failure mode — `Blade.Runner.2049.2017` answering with the 1982 film — so a match whose release year is more than a year from the one the filename states is rejected, except where the number opens the name and is the title rather than a year (`2001.A.Space.Odyssey`, `1917`).
- Save the mount directories when the scan is started rather than from its success handler, so a scan that fails no longer throws away what the operator typed — which is exactly when the paths matter most. A save failure is remembered and appended to the scan's own result message instead of being overwritten when the scan finishes minutes later.
- Keep the Scan Files buttons busy while a scan is actually running. Their state came from the local mutation, so navigating away and back reset them to idle mid-scan and invited a second scan of the same library. The running scan is now tracked on the server and exposed as `GET /scan-status`, which both buttons poll, so a scan started before the page remounted — or from another tab — still reads as running. Only one scan runs at a time; a concurrent request gets a 409 carrying a localized `scanAlreadyRunning` errorKey.
- Stop D Mode showing the poster behind the picture once playback has started, and label its loading state the way the standard player does. The canvas is painted opaque only where the frame reaches, so anything left behind it shows through the letterbox: fullscreen, where the container is the viewport rather than the video's own shape, the poster was contain-fitted to that and stood around the picture for the whole video; docked it leaked a rounding-wide sliver, usually along the bottom, because the box height and the canvas's `object-fit` result are fitted to the decoded ratio separately. The poster now covers only what it exists for — the wait for the first frame — keyed off `aspectRatio`, which the engine sets as that frame is painted and which returns to null with the fresh engine each source gets, so the next video is still covered while it loads. The "Preparing D Mode playback..." string is replaced by the player-wide `loadingVideo` ("Loading video..."), so a viewer sees the same wording whichever player they are on, and the now-unused key is dropped from every locale.
- Stop a scanned series being offered out of order. Scan files are processed three at a time, so a folder's videos were linked to its collection in whatever order ffprobe and TMDB happened to finish them in, and that arrival order is what the collection stores. Up Next reads it twice - once to pick the video that follows the one playing, once to score the rest by their distance from it - so an eight-episode show opened on its first episode offered E03, E02, E04, E05, E07, E06, E08. Every file is now numbered by its place in the naturally sorted folder listing, and the folder is laid out in that order once the whole pass has finished - not as each file lands, because an insert can only aim at a position the list has already grown to, so a folder finishing 3, 2, 1 would still be stored as 1, 3, 2. That is a single write per collection, skipped entirely when the stored order already matches, so a re-scan of a settled library pays nothing and a large one never goes near the per-video rebuild path. Collections imported by an earlier scan are straightened out by the next one. The player's own fallback for a series that never earned a collection is fixed too - it grouped episodes by a stem built from the title tokenizer, which drops CJK outright while keeping `s01e01`, leaving a Chinese show with either an empty stem or a different one per episode and so no next episode at all; the stem now removes the episode designator and keeps the rest of the name whatever script it is written in, flattening separators first so an underscored `Show_S01E01` - where the designator has no word boundary to be found by - reads the same as a spaced one.
...
- Serialize D Mode's playback starts against its seek queue, so a slow one can no longer be cancelled by a seek that began underneath it. `seek()` records whether playback was running as it starts and restores that on the way out, and `play()` can sit in `resumeClock()` for over a second waiting on the audio context, so a release made while autoplay or a deferred play press was still starting produced a seek that captured the pre-play state and put it back — leaving the clock and loops running behind a paused status. Autoplay and a deferred play are now performed by the same queue that owns repositions rather than beside it: one engine call runs at a time, repositions first, with a transport change applied on top of the position the viewer chose. A press made while a play is still resuming composes against the state being reached rather than the one the engine is still reporting, so a second press stops it instead of being swallowed. Skips no longer need a pending target to survive: one pressed while anything else is under way counts from the queued target, the target in flight, or failing both the last position the engine reported — which covers a skip during autoplay startup and a skip during another skip, both of which used to be dropped. A skip pressed while an ended video is rewinding to replay counts from the zero that rewind lands on.
- Fix the D Mode progress bar, which ignored the theme and could not be dragged. Its transport strip was hardcoded light-on-dark, so it stayed white-on-black in light mode rather than following the app the way the standard player's control panel already does; docked in the page it now takes the theme's elevated surface and text colours, and keeps the light-on-dark treatment only in fullscreen, where it sits on the player's own black frame. The bar itself was a static readout — two nested boxes with no pointer handlers at all, able to report the position but not to set it — and is now the same shared `ProgressBar` the standard player uses, which brings drag, touch and keyboard seeking along with that colour scheme instead of duplicating it. Two details are specific to D Mode: the dragged position is held locally so playback's own position updates cannot pull the thumb back out from under the pointer, and because the engine ignores a `seek()` issued while another is still running, releases go through a one-slot queue where the newest target wins and is performed once the in-flight seek finishes — otherwise a quick second drag was silently dropped. That queue covers every path that moves the playhead, not just the bar: because the engine refuses an overlapping seek without reporting it, a release made while the saved-position restore or a skip button was still running used to be lost, and a skip made during a drag now counts from the position the drag is heading for rather than from the clock, which still reads the position being left. The queue is bound to the engine that owns it, and is reset with the rest of the bar's state when the source changes, so a target aimed at one video can never land on the next one — the player stays mounted across navigation. A release queued during the saved-position restore is also finished before autoplay starts, since it would otherwise capture the paused state and put it back once it landed. Pressing play on an ended video holds the queue too, because the engine rewinds such a source with a seek of its own. Play and pause pressed while the playhead is moving are held and applied once it lands, because `seek()` restores the playback state it captured when it started — pausing mid-seek was undone on the way out, and starting playback mid-seek left the clock running behind a paused status. Seeks commit on release rather than continuously, since each one resets both decoders and re-demuxes from a keyframe.
...
- Pin qs to 6.16.0 through an `overrides` entry, fixing two medium-severity advisories reachable through express's query-string and urlencoded body parsing: an array limit that `parseArrayValue()` failed to enforce for bracket-key comma input such as `a[]=1,2,3,4` (CVE-2026-82562), and an uncaught `TypeError` thrown out of `isBuffer()` for a crafted `x[constructor][isBuffer]=y` key (CVE-2026-82417). Neither issue is in express itself — `express@4.22.2` declares `qs: ~6.15.1`, which caps resolution below the fix — so the override reaches 6.16.0 without the express 4 -> 5 major upgrade the advisory proposed. (#428)

## v1.11.1 (2026-08-30)

### Feat

- Add **D Mode**, an opt-in in-car playback path that renders video with no media element at all: the container is demuxed in JavaScript (ISO-BMFF and Matroska/WebM, including all three lacing modes), frames are decoded with WebCodecs and painted into a `<canvas>`, and audio is decoded into `AudioBuffer`s scheduled on a Web Audio graph — so the page holds no `<video>`/`<audio>`/`<source>`/`<track>` node and creates no MediaSource. This lets video play on in-car entertainment screens that block ordinary players while the vehicle is moving; it is intended for passengers, and the README says so. The audio clock is the master clock, so pause and resume work through `AudioContext` suspend/resume without drift, with a wall clock for video-only files. Includes seeking with saved-progress resume, transport controls and a fullscreen toggle drawn over the picture and scaled with it, and terminal in-canvas failure reporting; a quadratic MP4 sample-table build was also fixed (15.5s -> 89ms). MKV/AVI and fragmented MP4 fall back to the standard player with a message, and subtitles, playback speed and live translation are unavailable in D Mode. (#424, #425)

## v1.11.0 (2026-08-24)

### Feat

- Publish yt-dlp updates as immutable managed releases, so an update can no longer mutate the Python environment a running download is reading from. Each update installs into a fresh staging directory with `pip --target` and is validated there — version, `--help`, impersonation probes, and a module-origin check that asserts `yt_dlp` resolves inside the candidate and rejects any optional dependency resolving from an ambient install — then is renamed into `releases/<id>` and never written to again. Publication reserves its generation with an `O_EXCL` create, making the pointer update a compare-and-swap rather than a check followed by a write, and `current.json` is replaced by a single atomic rename over an fsynced directory. One logical operation acquires one immutable release and keeps it across retries, so a job can never combine one release's capabilities with another release's executable, and every yt-dlp process now spawns through `spawnYtDlp` / `withYtDlpRelease`, enforced by an AST rule that counts process-creation calls per module. Not a breaking change: with no store present, resolution falls through to `YT_DLP_PATH`, the image-pinned binary, then PATH discovery exactly as before, the store is created lazily on first install, and any store problem — corrupt, unwritable, symlinked, or unable to hand out a lease — degrades to discovery rather than preventing the backend from serving. Each retained release costs ~50-100 MB. (#420)
...
- Stop the yt-dlp release collector from deleting a retirement that is still in flight, and from freeing a generation whose record survived removal. Collection retires a release by renaming it into the trash, then re-checks for late leases and renames it back if a reader got there first; `emptyTrash()` deleted every trash entry unconditionally, so a concurrent collector could remove a retirement still waiting on that re-check and leave a reader holding a legitimate lease running against deleted modules — the exact failure the retirement protocol exists to prevent. Entries whose token is still a live marker are now skipped, and abandoned ones are swept as before. Separately, reclaiming an abandoned generation removed the dead publisher's `published.json` but ignored whether that removal succeeded, so a surviving record plus a freed generation could leave two releases recording the same generation and corrupt both recovery ordering and the rollback window; the stale-reclamation path now puts the claim back so a later attempt retries the removal. (#422)

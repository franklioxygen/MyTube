...
- Add an opt-in `Author → playlist seasons` media-server export layout that builds a separate, MyTube-managed TV library under `uploads/media-library/`: one show per author, one numbered season per source-backed playlist, and `Season 00` (*Specials / Unassigned*) for videos in no playlist, with `tvshow.nfo`, `season.nfo`, per-episode NFOs, and show artwork. Season and episode numbers are allocated once and never renumbered — an upstream playlist reorder only records the new position — and a video that belongs to several playlists becomes a separate episode in each. Original files are never moved, renamed, or duplicated: episodes are hard links back into `uploads/videos/`, so the library normally consumes no additional disk space. Where hard links are impossible (a different filesystem, or one that cannot link) MyTube copies instead, which does consume a second full copy of each affected video; the rebuild summary reports the linked-versus-copied counts and the fallback can be turned off. Cleanup is driven by a database ownership ledger and deletes only files MyTube generated — files you added to the library yourself are preserved and reported rather than overwritten. The default remains `Adjacent sidecars`, so existing installations are unaffected until they opt in. Add `uploads/media-library/` (not `uploads/videos/`) to Jellyfin, Plex, Emby, or Kodi as a Shows library with local NFO metadata enabled. Closes #411
- Let a collection be exported as its own media-server show instead of a season under its author. In the `Author → playlist seasons` layout a collection is normally a season, so a drama series posted by an uploader shows up in Plex under the uploader's channel name and avatar. Opening a collection now offers **Export as its own show**, which gives it a top-level show directory with a single `Season 01`, its own `tvshow.nfo`, and its own poster; the author's show is left untouched. The title comes from one of three explicit choices — search TMDB, type it manually, or keep the collection's own name — and MyTube never picks for you: TMDB matches that fail a strict check are labeled suggestions and still require confirmation, and a TMDB API key is needed only for the search path. TMDB supplies the show's identity, artwork, and premiere date only; episode numbers keep coming from MyTube's own allocator, never from TMDB. The toggle is opt-in per collection and appears only in the managed-library layout. Note that promotion is not losslessly reversible: promoting a collection that was already a season removes the old season directory and retires its season number (episode numbers are carried across, so `SxxExxx`-keyed watch state survives), and turning the toggle back off returns the collection to the author show as a new season with a new number. To improve only the title of an existing season, rename the collection instead.
...
- Say what the managed-library rebuild actually does before it runs. In the `Author → playlist seasons` layout the button was still labelled "Rebuild Media Server Sidecars" and its confirmation described rewriting NFO and artwork files next to existing videos — accurate for the `Adjacent sidecars` layout, and badly misleading for the managed one, where the same click materializes the *entire* library into `uploads/media-library/` and creates one show folder per author. A user could add dozens of shows to their media server expecting a metadata refresh. The button is now "Rebuild Managed TV Library", the confirmation says the whole library is rebuilt rather than just recent downloads, and it states the concrete size of the run — how many videos will be processed and roughly how many show folders will result — from a new read-only scope endpoint that reuses the reconciler's own show-identity resolution, so the number cannot drift from what the allocator produces.
- Stop the managed TV library from renumbering episodes when the filename template or author-folder setting changes. A batch rename moved the original file and then dropped and re-created the episode assignment; the reallocation reused the video's recorded playlist position, which another episode may have taken after an upstream playlist reorder, so the video landed on a fresh number — `S01E001` became `S01E003`, the old file was deleted, and the media server saw an episode vanish and an unwatched one appear, losing watch state. Mirror paths never depended on the original filename, so the rename now relinks in place and leaves the catalog untouched. The same fix covers redownloads, which replaced the file behind a surviving library row through the identical path.

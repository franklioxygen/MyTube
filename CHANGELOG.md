...
- Make "Proxy only apply to Youtube" apply to proxies it does not own. The setting deleted the `--proxy` flag for non-YouTube URLs, which does nothing about `HTTP_PROXY`/`HTTPS_PROXY` in the environment: yt-dlp reads those itself, so a deployment that sets them went on proxying every non-YouTube download while the UI said otherwise. Non-YouTube URLs now get yt-dlp's explicit direct-connection value (an empty `--proxy`), which overrides the environment, and the rule runs even when no yt-dlp configuration text is set at all - that is precisely the deployment where the proxy comes from the environment rather than from the config. The empty value is meaningful rather than absent, so the two places that split network options out of the user config forward it instead of dropping it as falsy, and the Bilibili short-URL resolver disables axios' own environment-proxy handling to match, so the redirect chain and the download it feeds cannot take different egress paths. **Behaviour change:** if you enabled this toggle but relied on a container-level proxy to reach non-YouTube sites, clear it.

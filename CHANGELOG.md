...
- Retry failed subscription backfill videos by durably retaining each failed URL for the next scheduled check, even if newer uploads appear. Retries preserve the feed cursor, respect the configured media type, and add existing library videos to the subscription's playlist collection without downloading another copy. Settling an existing video still allows the independent Shorts check to run, and a Short the retry batch has already recovered is settled there rather than downloaded a second time. A recovered item keeps the position it held in the backfill, stored with the retry, so it is named like the siblings placed at 1..N instead of falling back to the renderer's index-less `00`, where several recovered items of one playlist would render alike and collide. This applies to failures recorded after the update; historical stuck subscriptions require manually downloading the missing videos and adding them to the playlist collection.

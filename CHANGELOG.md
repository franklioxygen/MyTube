...
- Coordinate the in-place yt-dlp update with running downloads. Serializing pip against itself only ordered the writers: a download could still spawn from — or lazily import out of — the installation pip was replacing, failing for a reason the user could not act on. New yt-dlp executions now wait for the duration of an update, and the update first gives already-running processes 30s to finish. The wait is bounded on purpose: an operator updates yt-dlp precisely when downloads are failing, so a long job cannot postpone the fix. The same coordination covers the automatic stale-version upgrade, and the MissAV downloader — which spawned yt-dlp directly, bypassing the shared helpers — now resolves the binary the same way as every other caller, so it also picks up auto-install and the provider plugin environment it was previously missing.

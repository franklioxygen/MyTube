...
- Retry a subscription video that the initial backfill failed to download. A playlist subscription seeds its cursor to the collection head at creation time, before the task that downloads the history has run, so a scheduled check does not fetch the same videos in parallel. If that task then failed on the head - one transient yt-dlp timeout is enough - the cursor was already past a video nothing had downloaded, every later check saw `head == lastVideoLink` and concluded there was nothing new, and the gap was permanent: the collection stayed one episode short, with no error left anywhere to explain it and no amount of waiting to fix it. A failed video on a subscription-linked task now clears that subscription's cursor, but only while the cursor still points at that video, so a check that has meanwhile moved on to a genuinely newer upload is not rewound underneath itself. Nothing else about failure handling changes: the scheduled check already retried an ordinary download failure by leaving the cursor where it was, and this extends the same treatment to the backfill. A check that finds its head already in the library now links it into the subscription's collection and advances the cursor instead of downloading a second copy - and carries on to the independent Shorts check rather than returning, since advancing the cursor also refreshes the check time and returning would have hidden a new Short for a full interval. The lookup is scoped to the media type the subscription's effective configuration would save under, the way the backfill's own duplicate check already is - which is what makes the rewind safe, since the retried video is fetched when it is missing and quietly settled when some other path has since supplied it.

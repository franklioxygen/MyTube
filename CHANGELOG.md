...
- Add an opt-in media-server export layout that builds a managed TV library at `backend/uploads/media-library`, where each author is a show and each source-backed playlist is a numbered season (`Show/Season NN/SxxExxx - Title`), complete with `tvshow.nfo`, `season.nfo`, per-episode NFOs, artwork and subtitles. Episodes are hard links to the originals, so the library normally costs no extra disk space; a copy fallback keeps it working on filesystems without hard links and the rebuild summary reports how many files were copied. Originals are never renamed or moved, filename settings do not affect the mirror, season and episode numbers are allocated once and never renumbered by an upstream reorder, a video in several playlists appears once per season, and videos in no playlist land in Season 00 as Specials. Every generated file is tracked in an ownership ledger, so cleanup and stale-file sweeping can never touch an original or a file you put there yourself. The default stays the existing adjacent-sidecar layout. Episode numbers are tombstoned when an occurrence is removed, so a freed number is never handed to different content — including when artifact cleanup fails on the way to deleting the video. A rebuild verifies each tracked mirror file against its source (hard link by inode, copy by size) rather than trusting that the path exists, so a truncated or swapped mirror file is repaired instead of reported unchanged. Only an actual deletion retires an episode number: a redownload, a batch rename or a file move refreshes the episode in place and keeps it. A show whose channel changed its display name gets the new title in `tvshow.nfo` and every `showtitle`, while its directory stays put, and a playlist started from the continuous-download endpoint becomes a season like any other rather than landing in Specials. A rename is taken from the download itself and is not overwritten by the older channel name a collection captured when it was created. A Short downloaded for a playlist subscription is exported immediately rather than waiting for a collection link that never comes, and a show that outlives one of its videos has its poster and premiere date replanned instead of keeping the deleted one's. Extractor fields captured at download time survive every later pass: planning reads each episode's preserved `.info.json` back from the mirror, so a rebuild, a metadata edit or a file move reproduces it instead of writing a synthesized envelope over it. Closes #411

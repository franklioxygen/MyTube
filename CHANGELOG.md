...
- Make Bilibili collection subscriptions notice new videos again. Two independent faults stacked, and either alone was enough to stall a subscription indefinitely. First, the poll's opening step - deriving the collection's owner and season id from the subscribed video URL - calls `api.bilibili.com/x/web-interface/view`, which now answers HTTP 412 (风控) to any request without cookies. The stored `cookies.txt` had only ever been handed to yt-dlp, so every direct API call went out unauthenticated and that derivation failed on every tick; the check is fail-closed, so the subscription recorded a failure and never reached the archive feed. The cookie file is now also read for the direct calls, matched to the request URL the way a browser would - by host (leading-dot and include-subdomains lines apply to subdomains, host-only lines do not), by path, and by expiry, each decided before a name is claimed so a scoped or stale row cannot hide the one that actually applies - and the request carries a complete browser user agent instead of the truncated string that was itself a risk-control signal. Second, the head of a collection was taken as the first archive the API returned, but a Bilibili collection is ordered by its author, not by upload date: with `sort_reverse` off that first entry is episode 1. A subscription created from such a collection therefore stored episode 1 as its cursor and then compared every later poll against it, so the head never changed and no new episode was ever seen - and the same wrong end was captured as the baseline at creation time. The head probe now reads every archive page and picks the newest publication time among them, which is also correct when an author appends an older video to a season or reorders it. The comparison uses the archive's raw timestamp rather than the YYYYMMDD upload date carried alongside it, because that date collapses to a UTC day: two archives published on the same day would tie and fall back to collection order, which is exactly the author-controlled ordering this stopped trusting. The day string is still what decides whenever either side carries no timestamp, since treating a missing one as older would let an archive lose to something it postdates. Reading a bounded window instead - the newest-first page, say - would have reproduced the same stall on any collection long enough for the newest upload to fall outside it, since Bilibili offers no date-sorted view of a collection and the newest upload can sit at any position. The scheduled poll therefore costs what the creation baseline already cost, one request per thirty archives. Download flows are untouched and still fetch in the collection's own order, so episode numbering and filename indices are unchanged. Finally, a poll that had to derive the source now stamps it onto the subscription's collection when that collection carries no source key, so subscriptions created before the key existed stop routing through the risk-controlled endpoint after one successful check. The stamp is withheld unless the subscription is the only one pointing at that collection: collections are shared by name, and once one carries a source key every subscription on it prefers that key over its own playlist id, so stamping would silently repoint the others - and since checks run concurrently, two of them could otherwise race to write different sources. A matching playlist id is not enough to call two subscribers equivalent, because a Bilibili source is the compound (platform, type, mid, id) the collections table is keyed on while a subscription row stores only the id half. A shared collection simply keeps deriving its source per poll, as before; the stamp only saves a request. The stamp itself re-reads the collection inside the write rather than saving the snapshot the poll has been holding across the archive scan, because saving a collection rebuilds every one of its video links from the object it is handed - so a stale snapshot would have dropped anything added meanwhile, including by the very download this poll was about to start.

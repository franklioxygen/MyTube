...
/** A database that predates migration 0029. */
function createPre0029Schema(): void {
  sqlite.exec(`
...
describe("media server export catalog migration", () => {
  ...
  it("applies the real migrations to a database that already has the catalog tables", () => {
    ...
    // earlier build of this feature: every statement in 0029 must be idempotent,
  ...
  it("creates the catalog tables and collection columns on a pre-0029 database", () => {

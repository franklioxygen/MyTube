...
export async function runMigrations() {
  try {
    ...
    if (!pathExistsSync(dbPath, [DATA_DIR])) {
      console.log(
    ...
    const hasLegacyData =
      pathExistsSync(VIDEOS_DATA_PATH, [DATA_DIR]) ||
      pathExistsSync(COLLECTIONS_DATA_PATH, [DATA_DIR]) ||
      pathExistsSync(STATUS_DATA_PATH, [DATA_DIR]) ||
      pathExistsSync(SETTINGS_DATA_PATH, [DATA_DIR]);

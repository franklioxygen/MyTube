...
export const deleteLegacyData = async (
  ...
  for (const file of filesToDelete) {
    ...
    if (fs.existsSync(file)) { // nosemgrep: javascript.pathtraversal.rule-non-literal-fs-filename
      try {
        // nosemgrep: javascript.pathtraversal.rule-non-literal-fs-filename
        fs.unlinkSync(file); // nosemgrep: javascript.pathtraversal.rule-non-literal-fs-filename
...
const persistAllowedHostsEnv = (
  ...
  try {
    ...
    // nosemgrep: javascript.pathtraversal.rule-non-literal-fs-filename
    fs.writeFileSync(envLocalPath, envContent, "utf8"); // nosemgrep: javascript.pathtraversal.rule-non-literal-fs-filename

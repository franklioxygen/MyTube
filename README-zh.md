...
- **D Mode（车载播放）**：使用 Canvas 和 WebCodecs 代替标准媒体元素，让视频可在车辆行驶时仍会屏蔽普通播放器的兼容车载娱乐屏幕上播放。此功能仅供乘客使用；驾驶员请遵守当地法规，切勿在驾驶时观看或操作屏幕。
...
- **媒体完整性检查**（管理员）：在 **设置 → 媒体库与存储** 中找出媒体库里文件缺失、被截断或无法读取的视频。它只报告问题，不做修改；每个问题都链接到对应视频，建议重新下载的还提供 **再次下载** 按钮。
...
- **键盘快捷键**：与 YouTube 一致的播放器快捷键——播放/暂停、跳转、音量、倍速、字幕、全屏、影院模式、逐帧以及接下来播放的切换。详见 [键盘快捷键](documents/zh/keyboard-shortcuts.md)。
- **RSS 订阅**：为外部 RSS 阅读器创建私密订阅链接，并可按频道、作者、标签、来源、最近时间范围和条目上限分别配置过滤条件。
- **登录保护与访客访问**：支持密码登录、可选通行密钥 (WebAuthn) 和仅管理员可用的 3×3 手势登录，并可创建具名只读访客账户。手势错误次数在所有设备之间共享；一次或两次错误会在最近一次错误的 12 小时后清零，三次错误则会锁定，直到管理员成功使用密码登录一次。
...
- **运行时**: Node.js 22.22+ 或 23.x–26.x，TypeScript
- **框架**: Express.js
...
```env
PORT=5551
# 可选：手势登录的稳定密钥（至少 32 字节）。
# 未设置时，MyTube 会在首次设置手势时创建 backend/data/gesture-login.pepper。
# GESTURE_LOGIN_PEPPER=请替换为至少32字节的随机值
# 可选：声明当前部署中管理员的信任边界。

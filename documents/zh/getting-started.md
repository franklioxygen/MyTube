...
- **Node.js** (22.22+、23.x、24.x、25.x 或 26.x)
- **npm** (v9 或更高版本) 或 **yarn**
...
5. **配置 Twitch 订阅** (可选):
   ...
   - 转到设置 → Twitch 订阅
   - 如果希望使用 Helix 进行频道识别和轮询，可填写 `twitchClientId` 与 `twitchClientSecret`
   - 凭据不是必需的；未填写时，MyTube 会回退到 yt-dlp 轮询的尽力模式
   - Twitch 订阅只会下载已发布的 VOD，不包含直播录制
...
8. **创建 RSS 订阅链接** (可选):
   ...
   - 转到设置 → 安全与访问
   - 为外部 RSS 阅读器创建私密订阅链接
   - 可为每个链接按频道、作者、标签、来源、最近天数和条目上限设置过滤条件
   - 请将 RSS 订阅 URL 视为密码保管；任何持有该 URL 的人都可以读取对应订阅

9. **开始下载**:
   - 在下载输入框中输入视频 URL
   - 支持的平台: YouTube、Bilibili、Twitch VOD、MissAV 以及所有 yt-dlp 支持的网站
...
- **视频下载跟踪**: 防止重复下载
- **订阅系统**: 从订阅的频道自动下载
- **RSS 订阅链接**: 为外部阅读器提供私密、可过滤的 RSS 输出
- **数据库迁移**: 启动时自动更新模式

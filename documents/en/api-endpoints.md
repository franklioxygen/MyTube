...
- `POST /api/settings/ytdlp/update` - Update yt-dlp in place via pip (no image rebuild required)
  ...
  - New yt-dlp executions are held for the duration of the update, since pip rewrites the installation they launch from. Processes already running get 30s to finish first; anything still running after that is left alone and the update proceeds, so a long download cannot postpone the fix indefinitely
